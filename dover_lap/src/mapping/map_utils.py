...
class HypothesisGraph:
    ...
    def initialize_cliques(self,
        ...
        if not init_map:
            ...
            for node_list in grouped_nodes.values():
                 self.cliques.append(
                     [self.node_index[node] for node in node_list]
                 )

    def search_cliques(self,
        max_iter: int
    ) -> float:
        ...
        max_weight = sum([self.__compute_clique_weight(clique) for clique in self.cliques])
        ...
        while (len(S_C)>0 and i < max_iter):
            ...
            choices = list(
                set([node for node,data in self.graph.nodes(data=True) if data['hyp']==hyp]) - set([x])
            )
            if len(choices) == 0:
                continue
            ...
            weight = sum([self.__compute_clique_weight(clique) for clique in self.cliques])
    ...
    def __compute_clique_weight(self,
        clique: List[int]
    ) -> float:
        clique_weight = sum(
            [data['weight'] for u,v,data in self.graph.edges(data=True)
